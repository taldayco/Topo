...
// Hash a seed into a large 2D offset to shift sampling away from the origin,
// breaking axis-aligned symmetry artifacts in simplex/cellular noise.
...
// Biased smoothstep: blend standard smoothstep with ease-out, weighted by bias.
// bias=0 -> pure smoothstep, bias=1 -> pure ease-out (favors high values).
...
void generate_worley_layer(std::vector<float> &out_value,
  ...
  // Edge proximity (distance2 - distance1)
