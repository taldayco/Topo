...
// Small region cleanup: merge quantized regions smaller than min_size into
// neighboring regions (same algorithm as the old noise.cpp).
...
void compose_layers(MapData &data, const ElevationParams &elev,
  ...
  // Ensure all layers use the same spatial scale
  ...
  for (int i = 0; i < n; ++i) {
    ...
    bool is_low = data.elevation[i] < comp.lava_threshold;
