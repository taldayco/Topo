...
void render_region_debug_overlay(std::vector<uint32_t> &pixels, int view_width,
                                 int view_height,
                                 const std::vector<UnusedRegion> &regions,
                                 std::span<const float> heightmap,
                                 int map_width, float offset_x, float offset_y,
                                 const IsometricParams &params,
                                 RegionType filter_type, uint8_t cr, uint8_t cg,
                                 uint8_t cb) {
  constexpr float ALPHA = 0.35f;

  for (const auto &region : regions) {
    if (region.type != filter_type)
      continue;
      ...
      for (int dy = -R; dy <= R; ++dy) {
        for (int dx = -R; dx <= R; ++dx) {
          ...
          if (px >= 0 && px < view_width && py >= 0 && py < view_height) {
            uint32_t overlay = 0xFF000000 | (cr << 16) | (cg << 8) | cb;
            pixels[py * view_width + px] =
                alpha_blend(overlay, pixels[py * view_width + px], ALPHA);
          }
        }
      }
    }
